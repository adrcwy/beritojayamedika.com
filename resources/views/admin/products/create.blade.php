<x-app-layout>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border-2 border-gray-100">
                ...
                <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
                    ...
                    <!-- Catalog Link - TAMBAHKAN INI -->
                    <div class="mb-6">
                        <label for="catalog_link" class="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                        ...
                        <input type="url" 
                            ...
                            placeholder="https://example.com/product-catalog.pdf"
                            value="{{ old('catalog_link') }}">
                        <div class="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <p class="text-xs text-gray-500">Isi URL INAPROC, lalu klik tombol scrape jika ingin ambil data otomatis.</p>
                            <button type="button" onclick="openScrapeFromField()" class="inline-flex items-center justify-center rounded-lg bg-indigo-50 px-3 py-2 text-xs font-bold text-indigo-700 hover:bg-indigo-100">
                                Scrape URL Ini
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        <label class="flex items-center gap-3 rounded-2xl border-2 border-green-100 bg-green-50/60 px-4 py-3">
                            <input type="hidden" name="is_active" value="0">
                            <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-green-600 focus:ring-green-500" checked>
                            <span>
                                <span class="block text-sm font-bold text-gray-800">Produk Active</span>
                                <span class="block text-xs text-gray-500">Matikan kalau produk belum ingin tampil di halaman publik.</span>
                            </span>
                        </label>
                    </div>
    ...
    <script>
        function openScrapeFromField() {
            const input = document.getElementById('catalog_link');
            const url = input ? input.value.trim() : '';
            window.open(url ? "{{ route('products.scrape.log') }}?url=" + encodeURIComponent(url) + "&auto_category=1" : "{{ route('products.scrape.log') }}", "_blank");
        }
    </script>
</x-app-layout>
